...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'injector-skills-opencode-'));
}
...
test('opencode fresh install drops plugin, commands, agents, skills, AGENTS.md, opencode.json', () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'injector-skills', 'plugin.js')), 'plugin.js missing');
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'injector-skills', 'package.json')), 'plugin package.json missing');
    assert.ok(fs.existsSync(path.join(ocDir, 'plugins', 'injector-skills', 'injector-skills-config.cjs')), 'injector-skills-config.cjs sibling missing');

    for (const f of ['injector-skills.md', 'injector-skills-commit.md', 'injector-skills-review.md', 'injector-skills-compress.md', 'injector-skills-stats.md', 'injector-skills-help.md']) {
    ...
    for (const name of ['injector-skills', 'injector-skills-commit', 'injector-skills-review', 'injector-skills-help', 'injector-skills-stats', 'injector-skills-compress', 'cavecrew']) {
    ...
    assert.match(agentsBody, /Respond terse like smart injector-skills/);
    // Block must be wrapped in begin/end markers so uninstall can isolate it
    // from user-authored content above and below.
    assert.match(agentsBody, /<!-- injector-skills-begin -->/);
    assert.match(agentsBody, /<!-- injector-skills-end -->/);
    ...
    assert.ok(cfg.plugin.includes('./plugins/injector-skills/plugin.js'), 'plugin entry missing');
  } finally {
...
test('opencode idempotent install does not duplicate plugin entries', () => {
  ...
  try {
    ...
    const matches = cfg.plugin.filter(p => p === './plugins/injector-skills/plugin.js');
    ...
    const sentinelCount = (agentsMd.match(/Respond terse like smart injector-skills/g) || []).length;
...
test('opencode re-install preserves user edits to plugin.js without --force', () => {
  ...
  try {
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'injector-skills', 'plugin.js');
...
test('opencode uninstall strips fenced AGENTS.md block, preserving user prefix and suffix', () => {
  ...
  try {
    ...
    // Sandwich the injector-skills block between user prefix and suffix.
    ...
    assert.doesNotMatch(after, /<!-- injector-skills-begin -->/, 'injector-skills block should be stripped');
    assert.doesNotMatch(after, /<!-- injector-skills-end -->/, 'injector-skills end marker should be stripped');
    assert.doesNotMatch(after, /Respond terse like smart injector-skills/, 'injector-skills body should be stripped');
...
test('opencode uninstall removes plugin dir, command/agent/skill files, prunes opencode.json', () => {
  ...
  try {
    ...
    const ocDir = path.join(xdg, 'opencode');
    assert.equal(fs.existsSync(path.join(ocDir, 'plugins', 'injector-skills')), false, 'plugin dir survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'commands', 'injector-skills.md')), false, 'injector-skills.md command survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'agents', 'cavecrew-builder.md')), false, 'cavecrew agent survived');
    assert.equal(fs.existsSync(path.join(ocDir, 'skills', 'injector-skills')), false, 'injector-skills skill dir survived');
    ...
    if (fs.existsSync(path.join(ocDir, 'opencode.json'))) {
      const cfg = JSON.parse(fs.readFileSync(path.join(ocDir, 'opencode.json'), 'utf8'));
      const stillHasPlugin = Array.isArray(cfg.plugin) && cfg.plugin.includes('./plugins/injector-skills/plugin.js');
...
test('opencode plugin handles /injector-skills ultra and stop injector-skills via tui.prompt.append', async () => {
  const xdg = freshTmpDir();
  const shimDir = shimOpencode();
  try {
    const env = { ...process.env, XDG_CONFIG_HOME: xdg, PATH: pathWith(shimDir), NO_COLOR: '1' };
    ...
    const pluginPath = path.join(xdg, 'opencode', 'plugins', 'injector-skills', 'plugin.js');
    const flagPath = path.join(xdg, 'opencode', '.injector-skills-active');
    ...
    const out1 = await handlers['tui.prompt.append']({ prompt: '/injector-skills ultra' });
    ...
    const out2 = await handlers['tui.prompt.append']({ prompt: 'stop injector-skills please' });
