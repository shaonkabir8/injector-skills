#!/usr/bin/env python3
"""Benchmark injector-skills vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "injector-skills" / "SKILL.md"
...
def load_injector-skills_system():
...
def run_benchmarks(client, model, prompts, injector-skills_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("injector-skills", injector-skills_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        injector-skills_medians = statistics.median(
            [t["output_tokens"] for t in entry["injector-skills"]]
        )
        savings = 1 - (injector-skills_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "injector-skills_median": int(injector-skills_medians),
    ...
    avg_injector-skills = round(statistics.mean([r["injector-skills_median"] for r in rows]))
    ...
    return rows, {
        ...
        "avg_injector-skills": avg_injector-skills,
    }
...
def format_table(rows, summary):
    ...
    for r in rows:
        ...
        lines.append(
            f"| {label} | {r['normal_median']} | {r['injector-skills_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_injector-skills']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark injector-skills vs normal Claude")
    ...
    injector-skills_system = load_injector-skills_system()
    ...
    results = run_benchmarks(client, args.model, prompts, injector-skills_system, args.trials)
