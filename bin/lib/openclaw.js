// injector-skills → OpenClaw install / uninstall helper.
//
...
// and 60K-total cap.
//
// To make injector-skills always-on through OpenClaw, we do two writes:
//   1. Drop a copy of skills/injector-skills/SKILL.md into <workspace>/skills/injector-skills/
...
const SKILL_NAME = 'injector-skills';
const SKILL_VERSION = '1.0.0';
const MARK_BEGIN = '<!-- injector-skills-begin -->';
const MARK_END = '<!-- injector-skills-end -->';
...
// injector-skills SKILL.md uses block-scalar `description: >`, which a naive split
...
function loadBootstrapSnippet(repoRoot) {
  if (repoRoot) {
    const p = path.join(repoRoot, 'src', 'rules', 'injector-skills-openclaw-bootstrap.md');
    const body = readIfExists(p);
    if (body) return body.endsWith('\n') ? body : body + '\n';
  }
  // Standalone fallback (curl|node case where there's no repo on disk).
  // Keep this in sync with src/rules/injector-skills-openclaw-bootstrap.md.
  return [
    MARK_BEGIN,
    ...
    '',
    "The full ruleset and intensity levels live in this workspace's injector-skills skill:",
    '',
    '  skills/injector-skills/SKILL.md',
    '',
    'Default intensity: `full`. Switch with `/injector-skills lite|full|ultra|wenyan`.',
    'Stop with: "stop injector-skills" / "normal mode" / "deactivate injector-skills".',
    '',
    'Auto-Clarity: drop injector-skills for security warnings, irreversible action',
...
function loadSkillBody(repoRoot) {
  ...
  return readIfExists(path.join(repoRoot, 'skills', 'injector-skills', 'SKILL.md'));
}
...
function installOpenclaw({ workspace, repoRoot, dryRun = false, force = false, log = noopLog() } = {}) {
  ...
  if (!skillBody) {
    log.warn('  openclaw install requires the injector-skills repo on disk (skills/injector-skills/SKILL.md missing).');
    log.note('  Re-run from a clone or via `npx -y github:JuliusBrussee/injector-skills -- --only openclaw`.');
  ...
  if (dryRun) {
    ...
    log.note(`  would ${fs.existsSync(soulFile) ? 'append to' : 'create'} ${soulFile} (injector-skills bootstrap block)`);
  ...
  else log.note(`  ${soulFile} already contains injector-skills bootstrap`);
...
function uninstallOpenclaw({ workspace, dryRun = false, log = noopLog() } = {}) {
  ...
  if (fs.existsSync(soulFile)) {
    if (dryRun) {
      log.note(`  would strip injector-skills block from ${soulFile}`);
      touched = true;
    } else {
      const r = stripBootstrapFromSoul(soulFile);
      if (r.changed) {
        log.note(r.removed ? `  removed ${soulFile}` : `  stripped injector-skills block from ${soulFile}`);
        touched = true;
      }
    }
  }
